...
- Buy submission is multi-position by design: up to `MINIMAL_MAX_CONCURRENT_POSITIONS` (default 3) concurrent entries are allowed across all market scopes. The hot-path guard is WSS-authoritative: it counts currently owned assets plus pending entry submits that are still `PENDING` or `UNKNOWN` while WSS ownership has not landed yet.
